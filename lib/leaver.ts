...
export function getGuilds(): Guild[] {
    const store = shelter.flux.stores.GuildStore as unknown as {
        getGuilds: () => Record<string, { id: string; name: string; icon: string | null; ownerId: string }>;
    };
 ...
 */
export function createLeaverQueue(delayMs: number = 5000, onComplete?: () => void | Promise<void>): Queue<LeaverQueueItem> {
    return createQueue({
        delayMs,
        maxRetries: 3,

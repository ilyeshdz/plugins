...
export function createQueue<T>(options: QueueOptions<T>) {
    ...
    const [state, setState] = createSignal<"idle" | "running" | "paused" | "completed" | "completed-with-errors">("idle");
    ...
    const updateItemStatus = (id: string, status: QueueItemStatus, error?: string) => {
    ...
    const addMany = (newItems: QueueItem<T>[]) => {
        setItems(prev => [...prev, ...newItems.map(item => ({ ...item, status: "pending" }))]);
    };

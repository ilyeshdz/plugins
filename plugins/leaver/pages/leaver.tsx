...
import { createLeaverQueue, getGuilds, type Guild } from "../../../lib/leaver.js";
...
export function LeaverPage() {
    ...
    const isRunning = createMemo(() =>
        ["running", "paused", "completed", "completed-with-errors"].includes(queue.state())
    );
    ...
    const allSelected = createMemo(
        () => filteredGuilds().length > 0 && filteredGuilds().every(g => selected().has(g.id))
    );
    ...
    const startLeaving = async () => {
        ...
        try {
            await openConfirmationModal({
                header: () => `Leave ${selectedGuilds.length === 1 ? "Server" : "Servers"}`,
                body: () => `Are you sure you want to leave ${selectedGuilds.length === 1 ? "this server" : `these ${selectedGuilds.length} servers`}?\n\n${serverNames}`,
                confirmText: "Leave",
                cancelText: "Cancel",
                type: "danger",
            });
            ...
            queue.addMany(selectedGuilds.map(g => ({
                id: g.id,
    ...
    const isQueueCompleted = createMemo(() => 
        queue.state() === "completed" || queue.state() === "completed-with-errors"
    );
        ...
        <div class={classes.page}>
            <Show when={!isRunning()} fallback={
                <div class={classes.runningState}>
                    <div class={classes.progressContainer}>
                        <div class={classes.progressBar}>
                            <div class={classes.progressFill} style={{ width: `${stats().percent}%` }} />
                        </div>
                    ...
                    <Show when={isQueueCompleted()}>
                        <Button size={ButtonSizes.MEDIUM} color={ButtonColors.PRIMARY} onClick={resetQueue}>
                            Done
                        </Button>
                    </Show>

                    <Show when={isQueueActive()}>
                        <Button size={ButtonSizes.MEDIUM} color={ButtonColors.RED} onClick={stopQueue}>
                            Stop
                        </Button>
                    </Show>
                </div>
                ...
                <div class={classes.header}>
                    ...
                    <Button size={ButtonSizes.MEDIUM} color={ButtonColors.SECONDARY} onClick={toggleAll}>
                        {allSelected() ? "Deselect all" : "Select all"}
                    </Button>
                </div>

                <Feedback type="warning">
                    Leaving many servers quickly may trigger rate limits or bans. Use responsibly.
                </Feedback>
                        ...
                        <For each={filteredGuilds()}>
                            {guild => {
                                ...
                                return (
                                    ...
                                    >
                                        {guild.icon ? (
                                            <img src={getGuildIconUrl(guild.id, guild.icon)} class={classes.guildIcon} alt={guild.name} />
                                        ) : (
                                            <div class={classes.guildIconPlaceholder}>{getInitials(guild.name)}</div>
                                        )}
                                        <div class={classes.guildName}>{guild.name}</div>
                                        <div class={classes.checkWrap}>
                                            <Checkbox checked={isSelected()} onChange={(checked: boolean) => {
                                                setSelected(prev => {
                                                    const newSet = new Set(prev);
                                                    checked ? newSet.add(guild.id) : newSet.delete(guild.id);
                                                    return newSet;
                                                });
                                            }} />
                                        </div>
                ...
                <Show when={selectedCount() > 0}>
                    <div class={classes.footer}>
                        ...
                        <Button size={ButtonSizes.MEDIUM} color={ButtonColors.RED} onClick={startLeaving}>
